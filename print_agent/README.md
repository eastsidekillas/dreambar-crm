...
- **Подключение:** `USB-агент` (ESC/POS-принтер) или `АТОЛ ККТ` (фискальник через ДТО 10)
...
заполнены backend_url, printer_id, agent_key и mode.
...
1. Установить «Драйвер ККТ 10» (ДТО 10) на кассовом ПК, подключить ККТ
   по USB и проверить связь в «Тест драйвера ККТ».
2. Положить рядом с агентом файл `libfptr10.py`. Инсталлятор ДТО его **не
   ставит** — он лежит в дистрибутиве драйвера: [fs.atol.ru](https://fs.atol.ru)
   → «Программное обеспечение» → ДТО → 10.x → `wrappers/python/libfptr10.py`.
   Работает и с собранным .exe — агент ищет файл рядом с собой.
   Важно: разрядность драйвера и Python/агента должна совпадать (x86 ↔ x86,
   x64 ↔ x64) — иначе fptr10.dll не загрузится.
...
Для режима `atol`: положите `libfptr10.py` рядом с `agent.py` **до сборки** —
build.bat вошьёт его в .exe, и на кассовом ПК отдельный файл не понадобится.
(Уже собранный .exe тоже умеет подхватывать `libfptr10.py` из своей папки.)
